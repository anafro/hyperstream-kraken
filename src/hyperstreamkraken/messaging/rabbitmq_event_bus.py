...
class RabbitMQEventHandler[E: RabbitMQEvent](ABC, Reified):
    @classmethod
    def get_event_type(cls) -> type[E]:
        return cast(type[E], cls.targ)

    def trigger(self, deliver: Basic.Deliver, message: bytes) -> None:
        event_class: type[E] = self.get_event_type()
        event = event_class(deliver=deliver, message=message)
        event.hydrate()
...
class RabbitMQEventBus:
    ...
    def start_listening(self) -> None:
        self.channel.basic_consume(
            queue=self.queue_name, on_message_callback=self.__consume
        )
    ...
    def __consume(
        ...
        try:
            ...
            event_handler.trigger(deliver=deliver, message=message)
        except Exception:
